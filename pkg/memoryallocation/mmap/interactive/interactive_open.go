...
func bookExample2() error {
	...
	// RSS ~= 4 MB
	b := make([]byte, 600*1024*1024) // after using make(...), RSS ~= 14 MB
	n, err := f.Read(b)
	if err != nil { // after f.Read(b), RSS ~= 600 MB (actually in mac m2, it's about 411 GB)
		return err
	}
	if n != len(b) {
	...
	*/
	...
	fmt.Println("Reading 5000 index", b[5000])      // RSS ~= 600 MB
	fmt.Println("Reading 100 000 index", b[100000]) // RSS ~= 600 MB
	fmt.Println("Reading 104 000 index", b[104000]) // RSS ~= 600 MB
	...
	return nil // still RSS ~= 600 MB (because OS uses page cache for any process tries to memorize the same file).
}
